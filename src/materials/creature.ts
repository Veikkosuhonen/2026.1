...
void main() {
  ...
  // Shape
  float a = (posOS.y+0.5);
  float w = max(0.0, (a*a - a*a*a));
  posOS.xz = posOS.xz * w * vec2(1.0, 1.5);
...
export const creatureMaterialInstanced = new THREE.ShaderMaterial({
  name: "CreatureMaterialInstanced",
  vertexShader: creatureShaderVS,
  fragmentShader: creatureShaderFS,
  uniforms: {
  ...
  userData: {
    materialKeys: [],
    attributes: [{ name: "color", size: 3 }, { name: "emissiveIntensity", size: 1 }],
  },
});

export const creatureMaterial = new THREE.ShaderMaterial({
...
creatureMaterialInstanced.onBeforeRender = (renderer, scene, camera: THREE.PerspectiveCamera, geometry, group) => {
...
creatureMaterial.onBeforeRender = (renderer, scene, camera: THREE.PerspectiveCamera, geometry, group) => {
  // const t = player.currentTime;
  // const bpm = player.bpm;
  // const bps = bpm / 60;
  // const beat = Math.floor(2 * t * bps);
