...
class CameraControls extends Controls {
  ...
  update( delta ) {
    ...
    this._targetLat = Math.max( - 85, Math.min( 85, this._targetLat ) );

    const smoothing = 1.0 - Math.exp( - this.lookSmoothFactor * delta );
    this._lat += ( this._targetLat - this._lat ) * smoothing;
    this._lon += ( this._targetLon - this._lon ) * smoothing;
...
function onPointerDown( event ) {
  ...
  if ( this.domElement !== document ) {
    console.log("Focusing domElement");
  ...
  if ( event.button === 2 ) {
    ...
    // Toggle pointer lock on right-click
    if ( this._pointerLocked ) {
...
function onPointerMove( event ) {
  ...
  this._targetLon -= event.movementX * this.lookSpeed;
  this._targetLat -= event.movementY * this.lookSpeed;
